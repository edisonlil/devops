# DevOps-Web 统一可视化管理平台

## 🎯 项目概述

### 背景与目标
- **背景**: 现有DevOps工具以CLI/脚本为主，运维新手使用门槛较高，缺少统一可视化入口来创建/管理模板、管理工作空间、发起与回溯部署作业
- **目标**: 提供一个轻量化、类Sealos风格的Web控制台，支持SSH远程连接，结合中间件部署的可视化管理，实现"远程管理、统一入口、简单易用、功能强大"

### 核心理念
**"SSH远程连接 + 工作空间为入口 + 模块化管理 + 可视化配置 + 智能推荐"**

通过SSH连接远程主机，读取远程workspace配置，基于Sealos风格的桌面式用户体验，以工作空间为统一入口，提供中间件部署、应用管理、模板管理等模块化功能，同时保持与现有CLI系统的完全兼容性。

### 用户旅程
```
SSH登录远程主机 → 读取workspace/enable配置 → 进入默认工作空间控制台 → 显示资源管理卡片
```

**说明**：
- 一次会话管理一台主机，切换主机需要登出重新登录
- 登录成功后自动读取远程主机的workspace/enable文件确定默认工作空间
- 如果enable文件不存在，弹出工作空间选择器让用户选择并生成enable文件

## 🏗️ 设计原则

### 1. 可视化优先
- 向导式流程、所见即所得参数表单
- 桌面式应用布局，直观的模块导航
- 实时预览和智能推荐

### 2. 轻量落地
- 前端静态资源 + 极薄后端（仅做文件系统读写、进程调用、日志转发）
- 与现有命令对齐：所有操作均映射到现有`bin/*.sh`、`devops`命令
- 数据直接读写`workspace/`、`templates/`等目录，最大化兼容性

### 3. 工作空间隔离
- 多租户支持，工作空间级别的完全隔离
- 基于工作空间的权限控制和资源管理
- 统一的配置策略和默认值管理

### 4. 安全默认
- 最小权限执行、敏感变量脱敏展示
- 操作审计和历史追踪
- 基于角色的权限控制

## 👥 用户角色

### 运维新手
- SSH连接远程主机，零命令行操作
- 通过桌面式界面点击创建/部署/回滚
- 可视化配置表单，智能推荐和验证
- 实时查看部署进度和日志

### 进阶运维/开发
- 管理多个远程主机和工作空间
- 自定义模板参数、批量环境管理
- 工作空间级别的配置管理
- 灰度/分批发布（后续迭代）

### 管理员
- 远程主机连接和权限管理
- 全局模板管理和维护
- 工作空间权限与审计
- 外部平台集成参数维护

## 🎨 界面设计

### 设计理念

参考现有工作空间首页的简洁风格（如img.png所示），保持一致的设计语言和用户体验。

**核心原则**：
- **极致简洁**: 干净的白色背景，清晰的层次结构
- **内容优先**: 信息层次清晰，突出重要内容  
- **克制装饰**: 图标使用克制，不过度装饰
- **一致体验**: 保持与现有页面的视觉统一性

### 主要页面设计

#### 1. SSH登录页面
- **布局**: 居中对话框，简单表单设计
- **色彩**: 保持与现有系统一致的蓝色主色调
- **交互**: 清晰的错误提示，简单的加载状态

#### 2. 工作空间控制台（使用现有设计）
- **顶部导航**: 保持现有设计，显示主机信息和工作空间切换器
- **统计信息**: 保持现有的数字显示风格
- **功能卡片**: 沿用img.png中的卡片设计，简洁的蓝色块状图标

#### 3. 其他页面设计原则
- **中间件管理**: 沿用卡片列表设计，简单的状态显示
- **参数配置**: 干净的表单设计，清晰的分组和标签
- **部署进度**: 简单的进度条和日志显示

### 图标使用规范

**克制原则**：
- 仅在必要时使用图标，避免过度装饰
- 优先使用文字和数字传达信息
- 图标风格统一，使用简单的线性图标

**使用场景**：
- 功能入口：仅在主要功能卡片上使用简单图标识别
- 状态显示：使用颜色块或文字显示状态，少用图标
- 操作按钮：优先使用文字按钮，避免纯图标按钮

**禁止使用**：
- 过多的装饰性图标
- 复杂的多彩图标
- 功能意义不明确的抽象图标

### 核心模块说明（保持现有UI风格）

- **中间件管理**: 中间件的可视化部署、管理和监控（数据库、缓存、消息队列等）
- **应用管理**: 应用的部署和生命周期管理
- **远程部署**: 远程执行devops命令，管理多服务器部署
- **数据库**: MySQL、PostgreSQL、MongoDB 等数据库服务
- ~~**监控告警**: 系统监控、性能分析和告警通知~~ *(后续版本)*
- ~~**CI/CD**: 持续集成和持续部署流水线~~ *(后续版本)*
- ~~**存储管理**: 对象存储、文件系统和数据备份~~ *(后续版本)*
- ~~**网络服务**: 负载均衡、域名解析和网络安全~~ *(后续版本)*

## 🔧 功能架构

### 1. 中间件管理模块

#### 用户操作流程
```
选择模板 → 配置参数 → 预览部署 → 执行部署
```

#### 模板选择页面
- 智能分类：按中间件类型分类
  - **数据库类**: MySQL、PostgreSQL、MongoDB、Redis等
  - **消息队列**: Kafka、RabbitMQ、RocketMQ等
  - **搜索引擎**: Elasticsearch、Solr等
  - **缓存系统**: Redis、Memcached等
  - **其他中间件**: Nginx、Tomcat、Zookeeper等
- 难度标识：简单✨、推荐⭐、中级、高级标识
- 搜索过滤：支持名称搜索和多维度过滤
- 工作空间模板：显示工作空间级别的自定义模板

#### 参数配置页面
- 资源滑块：直观的CPU、内存资源选择
- 智能推荐：根据工作空间和集群资源自动推荐配置
- 实时验证：参数输入时实时验证格式和合理性
- 成本预估：实时计算和显示预估成本
- 工作空间默认配置：自动应用工作空间级别的默认配置

#### 部署预览页面
- 配置摘要：清晰展示所有配置信息
- CLI命令：显示等效的命令行操作
- 网络信息：预览连接地址和访问方式
- 一键复制：支持复制CLI命令和连接信息

#### 部署执行页面
- 进度跟踪：实时显示部署进度和状态
- 日志流：实时显示部署日志
- 时间预估：智能预估剩余部署时间
- 错误处理：部署失败时提供详细错误信息和解决建议

### 2. 工作空间管理

#### 工作空间概览
- 显示每个工作空间的中间件数量和状态
- 实时监控运行中和异常的实例数量
- 成本统计和预算控制
- 权限控制：只显示用户有权限访问的工作空间

#### 配置管理
- 可视化编辑工作空间配置和部署文件
- 支持YAML校验、版本记录
- 变量/凭证管理：敏感值脱敏显示
- 默认配置策略：工作空间级别的默认配置

### 3. 全局模板管理

#### 模板中心
- 模板列表：按类型（k8s、swarm、vue、spring-boot等）与来源（全局/空间）过滤与搜索
- 详情与参数：展示关键结构，参数说明与默认值
- 一键创建：向导式填写参数 → 渲染预览 → 选择工作空间 → 落盘

#### 模板编辑器
- 可视化模板编辑器
- 参数定义和验证规则
- 模板预览和测试
- 版本管理和发布

### 4. 部署作业管理

#### 作业执行
- 发起作业：选择模板/工作空间与参数，调用现有命令执行
- 实时日志：WebSocket/SSE转发子进程stdout/stderr
- 历史记录：记录在本地元数据文件中
- 回滚/重试：复用历史参数，调用相应命令执行

#### 集成能力
- Kubernetes：命名空间/服务暴露向导，字段直接映射deploy.yaml
- Harbor：展示镜像仓库与推送信息

## 🏛️ 技术架构

### 前端技术栈
```
Vue 3 + Naive UI + TypeScript + Vite
```

### 目录结构
```
src/
├── views/
│   ├── workspace/
│   │   ├── WorkspaceHome.vue         // 工作空间首页（桌面式）
│   │   └── WorkspaceSettings.vue     // 工作空间设置
│   ├── middleware/
│   │   ├── MiddlewareManager.vue     // 中间件管理主页
│   │   ├── TemplateSelector.vue      // 模板选择页面
│   │   ├── ParameterConfig.vue       // 参数配置页面
│   │   ├── DeployPreview.vue         // 部署预览页面
│   │   └── DeployProgress.vue        // 部署进度页面
│   ├── application/
│   │   └── ApplicationManager.vue    // 应用管理主页
│   └── template/
│       └── GlobalTemplateManager.vue // 全局模板管理
├── components/
│   ├── workspace/
│   │   ├── AppGrid.vue               // 应用网格组件
│   │   └── WorkspaceHeader.vue       // 工作空间头部组件
│   ├── middleware/
│   │   ├── TemplateCard.vue          // 模板卡片组件
│   │   ├── ParameterForm.vue         // 参数表单组件
│   │   └── ResourceSlider.vue        // 资源滑块组件
│   └── common/
│       └── WorkspaceSelector.vue     // 工作空间选择器组件
├── composables/
│   ├── useWorkspace.js               // 工作空间管理
│   ├── useTemplates.js               // 模板管理
│   ├── useDeployment.js              // 部署管理
│   └── useValidation.js              // 参数验证
└── api/
    ├── workspace.js                  // 工作空间API
    ├── middleware.js                 // 中间件API
    └── template.js                   // 模板API
```

### 极薄后端
- **技术选型**: Node.js（Fastify）或 Python（FastAPI）
- **核心功能**:
  - **SSH连接管理**: 实现SSH客户端连接、会话管理、远程命令执行
  - **远程文件操作**: 读取/写入远程主机的`workspace/`、`templates/`目录
  - **进程执行器**: 远程调用`devops`与`bin/*.sh`命令
  - **日志流转发**: 实时转发远程命令输出（WebSocket/SSE）
  - **会话管理**: SSH连接状态维护、Session存储
  - **轻量元数据索引**: 本地JSON缓存远程主机信息

### 存储策略
- 不新增外部数据库；所有配置与状态均写回远程主机目录
- **远程文件访问**: 通过SSH读取/写入远程主机文件
- **会话存储**: SSH连接信息存储在服务器Session中
- **作业元数据**: 远程主机`workspace/.devops-web/jobs/*.json`
- **工作空间配置**: 复用远程主机`workspace/*/config`结构
- **enable文件**: 远程主机`workspace/enable`文件管理默认工作空间

## 🔌 API设计

### SSH连接认证
```javascript
POST   /api/auth/ssh-login                    // SSH登录认证
  {
    "host": "192.168.1.100",
    "username": "root", 
    "password": "password"
  }
POST   /api/auth/logout                       // 登出当前会话
GET    /api/auth/session                      // 获取当前会话信息
```

### 远程主机管理
```javascript
GET    /api/remote/workspaces                 // 获取远程主机的工作空间列表
GET    /api/remote/workspace/enable           // 读取远程workspace/enable文件
POST   /api/remote/workspace/enable           // 生成workspace/enable文件
  {
    "workspace": "default"
  }
GET    /api/remote/workspace/{name}/config    // 获取远程工作空间配置
```

### 工作空间管理
```javascript
GET    /api/workspaces/{workspace}/summary    // 获取工作空间概览信息
GET    /api/workspaces/{workspace}/defaults   // 获取工作空间默认配置
```

### 中间件管理
```javascript
GET    /api/workspaces/{workspace}/middleware/templates              // 获取模板列表
GET    /api/workspaces/{workspace}/middleware/templates/{name}       // 获取模板详情
POST   /api/workspaces/{workspace}/middleware/validate               // 验证配置参数
POST   /api/workspaces/{workspace}/middleware/deploy                 // 执行部署
GET    /api/workspaces/{workspace}/middleware/instances              // 获取实例列表
```

### 模板管理
```javascript
GET    /api/templates                         // 获取全局模板列表
POST   /api/templates                         // 创建新模板
PUT    /api/templates/{id}                    // 更新模板
DELETE /api/templates/{id}                    // 删除模板
```

### 作业管理
```javascript
POST   /api/jobs/deploy                       // 调用命令执行并返回作业id
GET    /api/jobs/{id}                         // 作业状态与元数据
GET    /api/logs/{id}/stream                  // 实时日志流
```

## 🛣️ 路由设计

### 路由结构
```javascript
const routes = [
  {
    path: '/login',
    name: 'Login',
    component: () => import('@/views/auth/Login.vue'),
    meta: { requiresGuest: true }
  },
  {
    path: '/',
    redirect: '/login'  // 未登录重定向到登录页
  },
  {
    path: '/workspace/:workspaceName',
    component: () => import('@/layouts/WorkspaceLayout.vue'),
    meta: { requiresAuth: true },
    children: [
      {
        path: '',
        name: 'WorkspaceDashboard',
        component: () => import('@/views/workspace/WorkspaceDashboard.vue'),
        meta: { title: '工作空间控制台' }
      },
      {
        path: 'middleware',
        name: 'MiddlewareManager',
        component: () => import('@/views/middleware/MiddlewareManager.vue'),
        children: [
          {
            path: 'deploy/:templateName?',
            name: 'MiddlewareDeploy',
            component: () => import('@/views/middleware/TemplateSelector.vue')
          },
          {
            path: 'config/:templateName/:instanceName?',
            name: 'MiddlewareConfig',
            component: () => import('@/views/middleware/ParameterConfig.vue')
          }
        ]
      },
      {
        path: 'application',
        name: 'ApplicationManager',
        component: () => import('@/views/application/ApplicationManager.vue')
      },
      {
        path: 'deploy',
        name: 'RemoteDeployManager',
        component: () => import('@/views/deploy/RemoteDeployManager.vue')
      },
      {
        path: 'template',
        name: 'GlobalTemplateManager',
        component: () => import('@/views/template/GlobalTemplateManager.vue'),
        meta: { requiresPermission: 'admin' }
      }
    ]
  }
]

// 路由守卫
router.beforeEach((to, from, next) => {
  const isAuthenticated = checkSSHSession()
  
  // 检查认证
  if (to.meta.requiresAuth && !isAuthenticated) {
    next('/login')
    return
  }
  
  // 检查游客访问（已登录用户不能访问登录页）
  if (to.meta.requiresGuest && isAuthenticated) {
    const defaultWorkspace = getDefaultWorkspaceFromSession()
    next(`/workspace/${defaultWorkspace}`)
    return
  }
  
  next()
})
```

## 🔐 权限与安全

### SSH连接安全
- **连接加密**: 所有SSH连接使用加密通信
- **凭据管理**: 密码不存储，仅保持会话期间的SSH连接
- **会话超时**: 自动断开长时间未使用的SSH连接
- **权限限制**: SSH用户只能访问项目目录下的文件和命令

### 权限模型（本期简化）
- **主机级权限**: 一个SSH用户对应一台主机的所有权限
- **工作空间访问**: 基于远程主机文件系统权限
- **模板管理**: 需要远程主机具备写入权限

### 安全措施
- **远程命令执行**: 只允许执行白名单内的devops相关命令
- **文件访问限制**: 只能读写项目目录下的文件
- **敏感信息脱敏**: 密码、密钥等敏感信息脱敏显示
- **操作审计**: 记录SSH连接、文件操作、命令执行等关键操作
- **网络安全**: 支持VPN环境下的SSH连接

## 🚀 实施计划

### 功能范围说明

#### 本期实现的核心功能 ✅
- **工作空间管理**: 工作空间首页、切换、基础管理
- **中间件管理**: 完整的中间件部署流程（选择模板→配置参数→预览部署→执行部署）
- **全局模板管理**: 模板的创建、编辑和维护

#### 本期暂不实现的功能 ⏸️
- **监控面板**: 资源监控和性能分析 *(后续版本)*
- **存储管理**: 存储卷和备份管理 *(后续版本)*
- **权限管理**: 用户和权限管理 *(后续版本)*
- **日志管理**: 日志查看和分析 *(后续版本)*
- **网络管理**: 服务暴露和网络配置 *(后续版本)*
- **应用管理**: 应用的部署和生命周期管理 *(后续版本)*

> **注意**: 暂不实现的功能在界面上会显示"暂未实现"标识，点击后提示功能开发中。

### MVP范围与里程碑

#### M1（2-3周）：基础功能 ✅ *本期实现*
- **SSH登录页面**: 实现SSH连接表单和认证功能
- **远程连接**: SSH客户端实现和会话管理
- **enable文件读取**: 读取远程主机默认工作空间配置
- **工作空间控制台**: 显示资源管理卡片界面
- **基础路由**: 登录守卫和路由重定向

#### M2（3-4周）：核心功能 ✅ *本期实现*
- **工作空间管理**: 远程工作空间列表和选择功能
- **中间件部署**: 模板选择、参数配置、部署预览、执行部署
- **远程命令执行**: 调用远程主机 devops 命令
- **实时日志**: WebSocket实时显示远程命令输出
- **enable文件管理**: 不存在时自动创建选择器

#### M3（4-6周）：高级功能 ⏸️ *后续版本*
- 全局模板管理和编辑器
- 应用管理模块
- ~~监控面板、存储管理、权限管理、日志管理、网络管理~~ *(暂不实现)*
- 分批/灰度发布 *(后续版本)*
- 定时/Webhook触发 *(后续版本)*
- 工作空间级权限与审计导出 *(后续版本)*

### 验收标准（MVP）
- **SSH登录功能**: 输入主机地址、用户名、密码成功登录远程主机
- **默认工作空间**: 登录成功后自动读取enable文件进入默认工作空间
- **中间件部署**: 选择Redis/MySQL模板，最少参数即能触发一次远程部署
- **实时日志**: 作业详情可实时查看远程命令输出日志
- **enable文件处理**: 不存在时弹出工作空间选择器，选择后自动生成
- **主机切换**: 通过登出重新登录实现主机切换

### 成功指标
- **SSH连接成功率**: 新用户首次SSH登录成功率 ≥ 95%
- **部署效率**: 新用户10分钟内完成人生第一次远程中间件部署 ≥ 70%
- **操作简化**: 常见部署相关问询量较CLI模式下降 ≥ 50%
- **网络稳定性**: SSH连接稳定性和断线重连成功率 ≥ 90%
- **多主机管理**: 支持通过登出/登录管理多台远程主机

## 🎯 核心优势

### 1. 统一的用户体验
- **桌面式首页**: 类似Sealos的直观桌面体验
- **工作空间上下文**: 始终明确当前操作环境
- **模块化导航**: 清晰的功能模块划分
- **一致的设计语言**: 统一的UI风格和交互模式

### 2. 企业级管理能力
- **多租户隔离**: 工作空间级别的完全隔离
- **权限精细控制**: 基于角色和模块的权限管理
- **资源统一管理**: 中间件、应用、模板的统一管理
- **操作审计追踪**: 完整的操作历史记录

### 3. 开发效率提升
- **模块化架构**: 清晰的代码组织结构
- **组合式API**: 可复用的业务逻辑
- **类型安全**: TypeScript提供的类型检查
- **组件复用**: 高度可复用的UI组件

### 4. 运维友好特性
- **实时监控**: 资源使用和状态监控
- **智能推荐**: 基于最佳实践的配置建议
- **成本控制**: 实时成本预估和预算管理
- **故障诊断**: 详细的日志和错误信息

### 5. 与现有系统完美兼容
- **配置互转**: Web配置可导出为CLI命令，CLI配置可导入到Web
- **模板同步**: 自动同步CLI模板到Web界面
- **一致性保证**: 相同的模板系统和部署逻辑
- **灵活切换**: 用户可根据场景选择Web或CLI方式

---

> **设计理念**: 本方案强调"界面轻、逻辑薄、命令重"。DevOps-Web负责可视化与引导，真实读取/写入仍在`workspace/`、`templates/`等既有目录，执行则复用已有devops命令与脚本，实现零迁移、低成本上线。同时通过工作空间为入口的设计，提供企业级的多租户管理能力。


# UI设计实施指南

## 设计原则（基于现有简洁风格）

基于img.png展示的现有界面风格，严格遵循以下设计原则：

- **极致简洁**：界面干净整洁，避免不必要的装饰元素
- **内容为王**：信息层次清晰，突出核心功能和数据
- **一致性优先**：与现有页面保持完全一致的视觉风格
- **克制设计**：图标、色彩、动效都保持克制，不过度设计
- **响应式布局**：确保在不同设备上的良好体验

## 实施细则

### 色彩方案（沿用现有）
```css
/* 基于现有页面的色彩 */
--primary-color: #007AFF;        /* 主色调蓝色 */
--background-color: #FFFFFF;     /* 纯白背景 */
--text-primary: #262626;         /* 主要文字 */
--text-secondary: #86868B;       /* 次要文字 */
--border-color: #E8E8E8;         /* 边框色 */
--card-background: #F8F9FA;      /* 卡片背景 */
```

### 字体系统（保持现有）
```css
font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'SF Pro Text', sans-serif;
```

### 布局规范（参考现有卡片）
- **卡片间距**: 20px
- **内容边距**: 24px  
- **圆角**: 12px（保持与现有卡片一致）
- **阴影**: 轻微阴影，不过度突出

### 图标使用（严格克制）
- **原则**: 仅在功能入口和状态标识处使用
- **风格**: 简单线性图标，与现有系统保持一致
- **禁止**: 复杂彩色图标、装饰性图标
- **优先**: 文字 > 数字 > 图标

### 交互设计（简洁优雅）
- **按钮**: 文字按钮为主，避免纯图标按钮
- **反馈**: 轻微的颜色变化和过渡效果
- **加载**: 简单的进度指示器
- **错误**: 清晰的文字提示，无多余装饰

### 组件规范
- **表单**: 简洁的输入框和标签，清晰的分组
- **列表**: 干净的行布局，适当的分割线
- **对话框**: 居中设计，简洁的按钮组
- **导航**: 保持现有顶部导航风格

## 开发注意事项

1. **严格遵循现有风格**: 所有新页面必须与img.png中的风格保持一致
2. **图标使用审查**: 每个图标的使用都需要考虑是否真正必要
3. **色彩一致性**: 严格使用定义的色彩变量，不随意添加新颜色
4. **动效克制**: 动效应该服务于用户体验，而不是装饰
5. **响应式考虑**: 确保在移动端也有良好的体验

通过严格遵循这些原则，确保新开发的功能与现有系统无缝融合，保持整体的简洁和专业感。