...
| `--static-dir` | string | - | 静态资源目录或tar包路径（支持.tar/.tar.gz/.tar.bz2） |
...
| 项目类型 | 默认模板 | 说明 |
|----------|----------|------|
| `java` | `spring-boot` | Spring Boot 应用模板 |
| `vue` | `vue-nginx` | Vue.js + Nginx 模板 |
| `nginx` | `nginx` | 纯 Nginx 静态网站模板 |
| `go` | `spring-boot` | 使用通用 Spring Boot 模板 |
| `tomcat` | `spring-boot` | 使用通用 Spring Boot 模板 |
| `python` | `python` | Python 应用模板 |
| `nodejs` | `nodejs` | Node.js 应用模板 |
...
# Nginx 静态网站（使用默认 nginx 模板）
devops run nginx website --static-dir ./dist/ --expose-port 30080

# Nginx 静态网站（指定其他模板）
devops run nginx website --static-dir ./dist/ --template vue-nginx --expose-port 30080
...
### 4. Node.js项目部署
```bash
# 基本Node.js项目部署（自动检测包管理器）
devops run nodejs \
  --git-url https://github.com/example/my-nodejs-app.git \
  --namespace production \
  my-nodejs-app
