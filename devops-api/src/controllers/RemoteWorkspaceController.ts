...
`;
  ...
  private generateConfigContent(configData: any): string {
    const lines = [
      ...
      `BUILD_GIT_USERNAME="${configData.gitUsername || ''}"`,
      `BUILD_GIT_PASSWORD="${configData.gitPassword || ''}"`,
      '',
      '# Harbor 配置',
      ...
      '',
      '# 构建配置',
      `BUILD_VERSION="${configData.buildVersion || ''}"`,
      `BUILD_COMMANDS="${configData.buildCommands || ''}"`,
